...
double prolonguedYGridG1[(numberGridPoints+1)/2+2*addGridPoints],prolonguedYGridG2[(numberGridPoints+1)/2+2*addGridPoints];
...
void defineGrid(double& discretization, string& gridSwitch)
{
	...
	double extendedLength(numberGridPoints*discretization),xValue(1e5);
	int counter(0);
	// Discretization is vector <double> with n+2*add elmnts
/////////////////////////////////////////////////////////////////////////////////////////////////////////	
	if(gridSwitch=="uniform")
	{
		for (int xGridIndex=0; xGridIndex < halfNumberOfPoints; ++xGridIndex)
		{
			 rightGrid.push_back(xGridIndex*discretization);
		}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////		
	if (gridSwitch=="quadratic")
	{
		...
		for (int xGridIndex=0; xGridIndex < halfNumberOfPoints; ++xGridIndex)
		{
			xValue = extendedLength*pow( ((1.*xGridIndex)/(1.*halfNumberOfPoints)),2.);
...
void setNumericalParameters(double& Length, double& initialDiscretization, double& error_tol, string& gridSwitch, double& p1_o_p2)
{
	...
	//angle_two_phi = 1.792; //slopeG1 = -0.8 
...
// for meiron problem, since slopes are not changed, not necessary yet to set prolongued splines for calculated slope in each step.
	double equation[numberGridPoints+1];	
	for (int i=0;i<numberGridPoints+1;i++) {equation[i] = 100.;}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		leftIntegral_conservationLaw = Integral;
 		...
 		gsl_integration_qng(&integrateExactDiffKernel,/*xGridG1[addGridPoints+1]*/0.00, tailFactor*xGridG1[addGridPoints+(numberGridPoints+1)/2-1],0,1e-7,&Integral,&error,&n_evaluations);
...
// 		exit(1);
		...
		gsl_integration_qng(&integrateExactDiffKernel,0.00/*xGridG1[addGridPoints+1]*/, tailFactor*xGridG1[addGridPoints+(numberGridPoints+1)/2-1],0,1e-7,&Integral,&error,&n_evaluations);
...
//////////////////////////////////////////////////////////////////////////////////////////
		...
		equation[i+(numberGridPoints+1)/2] = 0.5*(deltaG1-kappa*sigma)-(pG1/(2.*PI))*Integral;
...
{
	double d_x(1e-8);
	gsl_vector* equationVectorAt_x = gsl_vector_alloc(numberGridPoints+1);
	gsl_vector* equationVectorAt_xdx = gsl_vector_alloc(numberGridPoints+1);
	gsl_vector* xVector = gsl_vector_alloc(numberGridPoints+1);
	gsl_vector* xdxVector = gsl_vector_alloc(numberGridPoints+1);
	...
	for (int j=0;j<numberGridPoints+1;j++)
	{
...
// {
// 	cout << " in initializeSolutionVector 1: solutionG1_G2= " << *iter << endl;	
// }
// exit(1);
